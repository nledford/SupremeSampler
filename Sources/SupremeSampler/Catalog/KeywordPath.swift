...
/// ones `KeywordPathFilter.keywordPathsQuery` builds for a generated script.
struct KeywordPath: Equatable {
    ...
    let guid: String
    /// The keyword's own name, the path's last part.
    let name: String
    let text: String
    ...
    static func all(in tree: [CatalogPropNode]) -> [KeywordPath] {
        func walk(_ node: CatalogPropNode, parentPath: String?) -> [KeywordPath] {
            ...
            return [KeywordPath(guid: node.guid, name: node.name, text: text)]
