...
final class RandomSampleScriptGeneratorTests: XCTestCase {
    ...
    func test_givenAnyScript_whenGenerating_thenTheHeaderNamesTheAppAsPeopleSeeIt() {
        let script = header(for: [])

        XCTAssertTrue(script.contains("Generated by Supreme Sampler on "), script)
    }

    func test_givenKeywordCountRules_whenGenerating_thenTheHeaderSaysHowManyKeywords() {
        let script = header(for: [.keywordCount(.isEmpty), .keywordCount(.atLeast(2)), .keywordCount(.isNot(1))])

        XCTAssertTrue(script.contains("Keyword count filter: is 0 (no keywords)"), script)
        XCTAssertTrue(script.contains("Keyword count filter: is at least 2"), script)
        XCTAssertTrue(script.contains("Keyword count filter: is not 1"), script)
    }
