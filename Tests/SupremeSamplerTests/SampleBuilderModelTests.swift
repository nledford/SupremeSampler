...
final class SampleBuilderModelTests: XCTestCase {
    ...
    // MARK: - The rules being edited become the current filter

    private func rules(_ match: GroupMatch = .all, _ contents: RuleDraft.Content...) -> RuleGroupDraft {
        RuleGroupDraft(match: match, rules: contents.map(RuleDraft.init))
    }
    ...
    func test_givenACategoryRule_whenComputingCurrentFilter_thenIncludesIt() {
        let model = SampleBuilderModel.forTesting()
        model.rules = rules(.all, .keyword(KeywordRuleDraft(operator: .isAllOf, selectedGUIDs: ["g1", "g2"])))
    ...
    func test_givenAnyOfRules_whenComputingCurrentFilter_thenTheRootGroupMatchesAnyOf() {
        ...
        model.rules = rules(
            ...
            .rating(RatingRuleDraft(comparison: .exactly, value: 1)))

        XCTAssertEqual(
            model.currentFilter,
            SampleFilter(root: RuleGroup(match: .any, rules: [.rating(.exactly(5)), .rating(.exactly(1))])))
    }

    // MARK: - Selecting a parent category selects its whole branch

    /// Nature -> Pines -> Tall Pines, and Places -> Lake.
    private let branchTree = CatalogPropNode.buildTree(
    ...
    func test_givenAParentCategorySelected_whenComputingCurrentFilter_thenItsDescendantsAreIncluded() {
        ...
        model.rules = rules(.all, .keyword(KeywordRuleDraft(operator: .isAnyOf, selectedGUIDs: ["prop-pines"])))
    ...
    func test_givenAParentCategorySelected_whenGeneratingScript_thenTheScriptMatchesItsDescendants() {
        let model = SampleBuilderModel.forTesting()
        model.injectCatalogForTesting(FakeCatalog(), propTree: branchTree)
        model.rules = rules(.all, .keyword(KeywordRuleDraft(operator: .isAnyOf, selectedGUIDs: ["cat-nature"])))
    ...
    func test_givenTheSameSelection_whenGeneratingScriptRepeatedly_thenTheCategoryClauseIsIdentical() {
        ...
        func scriptBody(selecting guids: [String]) -> String {
            let model = SampleBuilderModel.forTesting()
            model.injectCatalogForTesting(FakeCatalog(), propTree: branchTree)
            model.rules = rules(.all, .keyword(KeywordRuleDraft(operator: .isAllOf, selectedGUIDs: Set(guids))))
            return model.generatedScript
                .split(separator: "\n")
                .filter { !$0.contains("Generated by SupremeSampler") }
                .joined(separator: "\n")
        }

        let reference = scriptBody(selecting: ["prop-lake", "prop-pines", "cat-nature"])
        for _ in 0..<20 {
            XCTAssertEqual(scriptBody(selecting: ["cat-nature", "prop-pines", "prop-lake"]), reference)
        }
    }
    ...
    func test_givenRatingAndCategoryRules_whenComputingCurrentFilter_thenIncludesBoth() {
        ...
        model.rules = rules(
            .all,
            .rating(RatingRuleDraft(comparison: .exactly, value: 5)),
            .keyword(KeywordRuleDraft(operator: .isAnyOf, selectedGUIDs: ["g1"])))
