...
The app's name as people see it is **Supreme Sampler**, and its one
source is `PRODUCT_NAME` in `project.yml`. Xcode derives the bundle's
file name, `CFBundleName` (the bold app-menu title) and the executable
(the process name) from it, and overwrites any other `CFBundleName`
while generating the Info.plist (checked 2026-09-26: neither an
`INFOPLIST_KEY_CFBundleName` nor a merged `INFOPLIST_FILE` survives).
**SupremeSampler**, no space, is the *code* name: module
(`PRODUCT_MODULE_NAME`, pinned so `@testable import SupremeSampler`
keeps working), target, scheme, project, repo and the DMG's file name
(no `%20` in download URLs). The test target's `TEST_HOST` is pinned
too, because XcodeGen builds it from the target name.

Don't add a `CFBundleDisplayName` or rename the bundle after building:
each is a second copy of the name. Both were tried, and left the menu
title, the DMG and the installed app disagreeing. Code that shows the
name (window title, catalog picker, the script header's "Generated by")
reads it back through `AppName.current`, and the Justfile reads
`app_name` from `project.yml`. `AppNameTests` (the test bundle runs
inside the built app, so `Bundle.main` is the real thing) and
`just verify-app` check the name. Releases up to v0.2.0 shipped as
`SupremeSampler.app`.
